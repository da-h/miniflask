...
def get_time_unit(unitvalue, targetunit):
    ...
    if targetunit == "m":
        unitvalue.data["m"] = unitvalue.data["s"] / 60
        return unitvalue.data["m"]
    if targetunit == "h":
        unitvalue.data["h"] = unitvalue.data["s"] / 60 / 60
        return unitvalue.data["h"]
    if targetunit == "d":
        unitvalue.data["d"] = unitvalue.data["s"] / 60 / 60 / 24
        return unitvalue.data["d"]
...
def set_time_unit(unitvalue, targetunit, newvalue):
    ...
    # remove all but s and computation_list whenever we change something
    unitvalue.data = {"s": unitvalue.data["s"], "computation_list": unitvalue.data["computation_list"]}
